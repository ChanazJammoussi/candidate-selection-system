...
const SECTIONS_BAC_TN = [
  ...
  "Économie et gestion", "Lettres", "Informatique", "Sport", "Sciences de l'informatique",
]
...
export default function CandidatureClient({ concoursNom, concoursType }: Props) {
  ...
  const [formData, setFormData]       = useState<Record<string, string>>({ email: "", telephone: "" })
  ...
  const updateAnneePrepa = (id: string, field: keyof AnneePrepa, value: string) =>
    setAnneesPrepa((prev) => prev.map((a) => {
      if (a.id !== id) return a
  ...
  const updateAnneeLicence = (id: string, field: keyof AnneeLicence, value: string) =>
    setAnneesLicence((prev) => prev.map((a) => {
      if (a.id !== id) return a
      const updated = { ...a, [field]: value }
      if (field === "resultat" && value === "refuse") { updated.session = ""; updated.rang = ""; updated.nbEtudiants = "" }
      return updated
    }))
  ...
  const requiredDocs: RequiredDoc[] = [
    ...
    ...(concoursType === "ing_licence" || concoursType === "master"
      ? anneesLicence.map((a) => ({
          id: `releve-licence-${a.id}`,
          label: "Relevé de notes — Licence",
          sublabel: [niveauLicenceLabel(a.niveau), a.anneeScolaire, a.etablissement ? `(${a.etablissement})` : ""]
            .filter(Boolean).join(" · ") || "Année non renseignée",
        }))
      : anneesPrepa.map((a) => ({
          id: `releve-prepa-${a.id}`,
          label: "Relevé de notes — Classes préparatoires",
          sublabel: [niveauPrepaLabel(a.niveau), a.anneeScolaire, a.etablissement ? `(${a.etablissement.split("—")[0].trim()})` : ""]
            .filter(Boolean).join(" · ") || "Année non renseignée",
        }))
    ),
  ]
      ...
      return (
        <Field key={champ.id}>
          <FieldLabel htmlFor={champ.id}>
            {champ.label}{champ.required && <span className="text-destructive ml-1">*</span>}
          </FieldLabel>
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">{concoursNom}</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="info">
          <Card>
            ...
            <CardContent>
              <FieldGroup>
                <div className="grid gap-4 md:grid-cols-2">{champsPerso.slice(0, 2).map(renderChamp)}</div>
                <div className="grid gap-4 md:grid-cols-2">{champsPerso.slice(2).map(renderChamp)}</div>
                <div className="grid gap-4 md:grid-cols-2">
                  <Field>
                    <FieldLabel htmlFor="email">Adresse e-mail <span className="text-destructive">*</span></FieldLabel>
                    <Input id="email" type="email" value={formData.email}
                      onChange={(e) => handleField("email", e.target.value)}
                      placeholder="prenom.nom@example.com" />
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="telephone">Téléphone <span className="text-destructive">*</span></FieldLabel>
        ...
        <TabsContent value="academic">
          <div className="space-y-6">
            <Card>
              ...
              <CardContent>
                <FieldGroup>
                  <div className="grid gap-4 md:grid-cols-2">
                    <Field>
                      <FieldLabel htmlFor="bacAnnee">Année d'obtention <span className="text-destructive">*</span></FieldLabel>
                    ...
                    <Field>
                      ...
                      <Select value={bac.section} onValueChange={(v) => setBac({ ...bac, section: v })}>
                        ...
                        <SelectContent>
                          {SECTIONS_BAC_TN.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </Field>
                  </div>
            ...
            {/* Parcours : licence ou prépa selon le type */}
            {(concoursType === "ing_licence" || concoursType === "master") ? (
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>Renseignez chaque année suivie, y compris les redoublements</CardDescription>
                    </div>
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <Field>
                          ...
                          <Select value={annee.anneeScolaire} onValueChange={(v) => updateAnneeLicence(annee.id, "anneeScolaire", v)}>
                            ...
                            <SelectContent>{ANNEE_SCOLAIRE_OPTIONS.map((a) => <SelectItem key={a} value={a}>{a}</SelectItem>)}</SelectContent>
                          </Select>
                        </Field>
                        <Field>
                        ...
                        <Field className="md:col-span-2">
                          ...
                          <Input value={annee.etablissement}
                            onChange={(e) => updateAnneeLicence(annee.id, "etablissement", e.target.value)}
                            placeholder="ex : Faculté des Sciences de Monastir" />
                        </Field>
                        <Field>
                          <FieldLabel>Moyenne générale <span className="text-muted-foreground text-xs">/20</span> <span className="text-destructive">*</span></FieldLabel>
                          <Input type="number" min={0} max={20} step={0.01} value={annee.moyenne}
                        ...
                        {annee.resultat === "admis" && (
                          <>
                            ...
                            <Field>
                              ...
                              <Input type="number" min={1} value={annee.rang}
                            ...
                            <Field>
                              ...
                              <Input type="number" min={1} value={annee.nbEtudiants}
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <Field className="md:col-span-2">
                          ...
                          <Select value={annee.etablissement} onValueChange={(v) => updateAnneePrepa(annee.id, "etablissement", v)}>
                            ...
                            <SelectContent>{ETABLISSEMENTS_PREPA.map((e) => <SelectItem key={e} value={e}>{e}</SelectItem>)}</SelectContent>
                          </Select>
                        </Field>
                        <Field>
                          <FieldLabel>Moyenne générale <span className="text-muted-foreground text-xs">/20</span> <span className="text-destructive">*</span></FieldLabel>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Indiquez si vous avez eu des années blanches depuis l'obtention du bac</CardDescription>
              </CardHeader>
              <CardContent>
                <FieldGroup>
                  <Field>
                  ...
                  {aAnneeBlanches === "oui" && (
                    <div className="grid gap-4 md:grid-cols-2">
                      <Field>
                        <FieldLabel>Nombre d'années blanches <span className="text-destructive">*</span></FieldLabel>
                        <Input type="number" min={1} max={5} value={nbAnneeBlanches}
                          onChange={(e) => setNbAnneeBlanches(e.target.value)}
                          placeholder="ex : 1" className="max-w-xs" />
                      </Field>
                      <Field>
                        <FieldLabel>Années concernées <span className="text-muted-foreground text-xs">(optionnel)</span></FieldLabel>
                        <Input value={anneesBlanches}
                      ...
                      <div key={doc.id} className="flex items-center justify-between p-4 rounded-lg border border-border">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {doc.sublabel && <p className="text-xs text-primary/80">{doc.sublabel}</p>}
                            <p className="text-sm text-muted-foreground">{uploaded ? uploaded.name : "Non téléversé"}</p>
                          </div>
                        </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {uploadedFiles.map((file) => (
                      <div key={file.id} className="flex items-center justify-between p-4 rounded-lg border border-border">
                        <div className="flex items-center gap-4">
                          <StatusIcon status={file.status} />
                          <div>
                            <p className="font-medium text-sm">{file.name}</p>
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{file.label}</span><span>•</span><span>{file.uploadDate}</span>
                            </div>
                            {file.feedback && (
                              <p className="text-xs text-primary mt-1">
                                <AlertCircle className="inline h-3 w-3 mr-1" />{file.feedback}
                              </p>
                            )}
                          </div>
                        </div>
