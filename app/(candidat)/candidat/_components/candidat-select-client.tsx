...
export interface ConcoursOuvert {
  id: string
  nom: string
  description: string
  typeLabel: string
...
const TYPE_COLORS: Record<string, string> = {
  "Cycle Ingénieur — Prépa":   "bg-blue-100 text-blue-700 border-blue-200",
  "Cycle Ingénieur — Licence": "bg-violet-100 text-violet-700 border-violet-200",
  "Concours Master":           "bg-green-100 text-green-700 border-green-200",
}

export default function CandidatSelectClient({ concours }: { concours: ConcoursOuvert[] }) {
        ...
        <div className="w-full max-w-2xl py-8 space-y-6">
          ...
          {concours.length === 0 ? (
            <div className="rounded-lg border border-dashed p-12 text-center">
              <p className="text-muted-foreground">Aucun concours ouvert pour le moment.</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {concours.map((c) => {
                const taux = c.places > 0
                  ? Math.min(Math.round((c.candidatsInscrits / c.places) * 100), 100)
                  : 0
                const color =
                  TYPE_COLORS[c.typeLabel] ?? "bg-secondary text-secondary-foreground border-border"
                ...
                return (
                  <Card
                    ...
                    onClick={() => {
                      ...
                      router.push(`/candidat/login?concoursId=${c.id}`)
                    }}
                  >
                        ...
                        <div className="flex items-start gap-4 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0 space-y-2">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <Badge variant="outline" className={color}>{c.typeLabel}</Badge>
                            </div>
                            ...
                            {c.specialites.length > 0 && (
                              <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
                                ...
                                <span className="truncate">{c.specialites.join(" · ")}</span>
                              </div>
                            )}
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span>Clôture : {c.dateCloture}</span>
                              </div>
                              <div className="flex items-center gap-1.5">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {c.candidatsInscrits} candidats inscrits
                            </p>
                          </div>
                        </div>
